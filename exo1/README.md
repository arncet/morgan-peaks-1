...
### Widget CLI
* Il est possible de faire un widget d'execution de code dans le browser en s'appuyant sur un serveur, mais l'infrastructure demandée peut devenir très vite pesante, surtout si le business-model impose la gestion d'importants pics de charge.
* On pourra soit créer une technologie maison, dédiée, soit s'appuyer sur une technologie open source comme TryItOnline (https://tio.run). L'intérêt d'utiliser une telle technologie est qu'ils proposent eux-même d'installer l'infrastructure nécessaire chez un hébergeur dédié comme DigitalOcean ou Vultr car c'est la solution qu'ils utilisent eux-même : https://github.com/TryItOnline/TioSetup
* Comme alternatives, on pourrait citer :
  * Atheos qui peut être lourd à héberger car écrit en PHP + JS : https://github.com/Atheos/Atheos
  * Replit qui n'est pas encore totalement OpenSource mais qui est prometteur https://github.com/replit
### Reverse proxy
